...
func main() {
	...
	case 0:
		fmt.Fprintf(os.Stderr, "ERROR: no name supplied\n")
		flag.Usage()
		os.Exit(1)
	case 1:
		name = flag.Args()[0]
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Send to %s failed: %s\n", net.JoinHostPort(*host, *port), err)
		return
	} else {
...
ReadServerResponse:
	...
	Switch:
		...
		case nil:
			break Switch
		case io.EOF:
			break ReadServerResponse
		default:
			fmt.Fprintf(os.Stderr, "Receive from %s failed: %s\n", net.JoinHostPort(*host, *port), err)
			return
		}
		fmt.Printf("%s", line)
	}
	...
	_debug("closing connection to server")
	conn.Close()
}
